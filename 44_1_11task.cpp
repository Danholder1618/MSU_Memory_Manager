...
Folder* CreateFolder(const char* name, Folder* parent)
{
	...
	if (newFolder) newFolder->parent = parent; else return nullptr;
...
File* CreateFile(const char* name, int size, Folder* parent)
{
	...
	if (newFile) newFile->parent = parent; else return nullptr;
...
void DeleteParsedPath(ParsedPath* parsedPath)
{
	for (int i = 0; i < countMemoryItemsSize; i++)
	{
		free(parsedPath->path[i]);
	}
	free(parsedPath->path);
	free(parsedPath);
}
#pragma endregion
...
int my_remove(const char* path, int recursive)
{
	...
	Folder* parentFolder = FindFolder(parsedPath);
	if (parentFolder == nullptr) return 0;
	const char* targetName = parsedPath->path[parsedPath->tokenCount];
	int isTargetDeleted = 0;
	for (int j = 0; j < parentFolder->sonsCount; j++)
	{
		if (strcmp(targetName, parentFolder->sons[j]->name) == 0)
		{
			Folder* target = parentFolder->sons[j];
			...
			{
				if (target->filesCount || target->sonsCount)
				{
					DeleteParsedPath(parsedPath);
					return 0;
				}
	...
	for (int j = 0; j < parentFolder->filesCount; j++)
	{
		if (strcmp(targetName, parentFolder->files[j]->name) == 0)
		{
			File* target = parentFolder->files[j];
			DeleteFile(target);
			isTargetDeleted = 1;
			break;
		}
